...
def _grader_catalog() -> list[Dict[str, Any]]:
    return [
        {
            "id": f"{tid}_grader",
            "task_id": tid,
...
async def get_graders():
    return _grader_catalog()


@app.get("/manifest")
async def manifest() -> Dict[str, Any]:
    manifest_text = (BASE_DIR / "openenv.yaml").read_text(encoding="utf-8")
    return yaml.safe_load(manifest_text)


@app.get("/openenv.yaml", response_class=PlainTextResponse)
async def openenv_yaml_file() -> str:
    return (BASE_DIR / "openenv.yaml").read_text(encoding="utf-8")


@app.get("/api/graders")
async def api_graders():
    return _grader_catalog()
...
@app.get("/api/tasks")
async def api_tasks_alias() -> list[Dict[str, Any]]:
    return _task_catalog()
...
async def grader_easy_alias(request: Request) -> Dict[str, Any]:
    return await grader_by_task("easy", request)


@app.post("/grader/medium")
async def grader_medium_alias(request: Request) -> Dict[str, Any]:
    return await grader_by_task("medium", request)


@app.post("/grader/hard")
async def grader_hard_alias(request: Request) -> Dict[str, Any]:
    return await grader_by_task("hard", request)
