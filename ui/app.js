...
function init() {
  G.vendors = [
    { id: "V1", quote: 320, status: "active" },
    { id: "V2", quote: 280, status: "active" },
    { id: "V3", quote: 350, status: "active" },
  ];
...
function addRew(val, reason) {
  addAF("c-rew", `Reward: ${val} (${reason})`);
}
...
function agentPolicy() {
  ...
  if (Math.random() < epsilon) {
    ...
    target = avail.reduce((best, curr) =>
      curr.quote < best.quote ? curr : best
    );
  }
